        ...
        for j in range(i, len(data.index)):
            if (i != j):  # TODO: change starting index to i+1 and remove this conditional
                ...
                for k in range(0, len(features)):
                    if (pd.notna(data.iloc[i, data.columns.get_loc(features[k])]) and
                        ...
                        if (data.iloc[i, data.columns.get_loc(features[k])] ==
                            ...
                            msg = msg.format(source=data.iloc[i, data.columns.get_loc("Code")],
                                             target=data.iloc[j, data.columns.get_loc("Code")],
                                             feature=features[k])
                            logger.debug(msg)
                            matches = matches + 1
                            if matched_features == "":
                ...
                if (matches >= (len(features) - allowed_misses) and
                    ...
                    nbr_target.append(data.iloc[i, data.columns.get_loc("Code")])
                    nbr_neighbor.append(data.iloc[j, data.columns.get_loc("Code")])
